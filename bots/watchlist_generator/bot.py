...
from scripts.genetic_fitness import best_fitness_map
...
class WatchlistGeneratorBot:
    ...
    def run(self, send_telegram: bool = True, trade_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _score_sentiment(s: Dict[str, Any]) -> float:
        if not s:
            return 50.0
        if s.get("status") in ("no_news", "model_unavailable"):
            return 50.0
        ...
        try:
            with db.get_session() as session:
                rows = session.execute(
                    text("SELECT symbol, phase, phase_confidence, status FROM market_data.stock_personalities")
                ).fetchall()
            return {
                str(r[0]): {"phase": r[1], "phase_confidence": r[2], "status": r[3]}
                for r in rows
            }
    ...
    def _load_features(self) -> Dict[str, Dict[str, Any]]:
        try:
            with db.get_session() as session:
                rows = session.execute(
                    text("SELECT symbol, ret_5d, rsi_14, volume_ratio_20, status FROM market_data.stock_features")
                ).fetchall()
            return {
                str(r[0]): {
                    "ret_5d": r[1],
    ...
    def _load_sentiments(self) -> Dict[str, Dict[str, Any]]:
        try:
            with db.get_session() as session:
                rows = session.execute(
                    text("SELECT symbol, sentiment_label, sentiment_score, status FROM market_data.news_sentiments")
                ).fetchall()
            return {
                str(r[0]): {
                    "sentiment_label": r[1],
